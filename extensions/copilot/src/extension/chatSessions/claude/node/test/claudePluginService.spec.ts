...
describe('ClaudePluginService', () => {
	...
	function createService(options?: {
		...
		const envService = new class extends NullNativeEnvService {
			override get userHome() { return options?.userHome ?? super.userHome; }
			override get appRoot() { return options?.appRoot ?? super.appRoot; }
		}();
	...
	it('includes bundled Sessions skills as a Claude plugin root when present', async () => {
		const appRoot = mkdtempSync(join(tmpdir(), 'claude-plugin-service-'));
		tempRoots.push(appRoot);
		const skillDir = join(appRoot, 'out', 'vs', 'sessions', 'skills', 'app-preview');
		mkdirSync(skillDir, { recursive: true });
		writeFileSync(join(skillDir, 'SKILL.md'), '---\nname: app-preview\ndescription: test\n---\n');

		const service = createService({ appRoot });
		const locations = await service.getPluginLocations(CancellationToken.None);

		expect(locations).toHaveLength(1);
		expect(locations[0].fsPath).toBe(join(appRoot, 'out', 'vs', 'sessions'));
	});
