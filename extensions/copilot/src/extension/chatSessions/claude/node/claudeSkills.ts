...
function getBundledSessionsPluginRoots(appRoot: string): URI[] {
	if (!appRoot) {
		return [];
	}

	const candidates = [
		URI.joinPath(URI.file(appRoot), 'out', 'vs', 'sessions'),
		URI.joinPath(URI.file(appRoot), 'src', 'vs', 'sessions'),
	];

	return candidates.filter(uri => existsSync(URI.joinPath(uri, 'skills', 'app-preview', 'SKILL.md').fsPath));
}
...
export class ClaudePluginService extends Disposable implements IClaudePluginService {
	...
	async getPluginLocations(token: CancellationToken): Promise<Uri[]> {
		...
		for (const uri of getBundledSessionsPluginRoots(this.envService.appRoot)) {
			pluginRoots.add(uri);
		}
