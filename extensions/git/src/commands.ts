...
import { createDesignerWorkspaceContext, DesignerWorkspaceContextReason } from './designerWorkspaceContext';
import { DesignerAutoSaveManager } from './designerAutoSaveManager';
...
const DesignerWorkspaceContextPreflightCommandId = '_designerWorkspaceContext.preflight';
const DesignerWorkspaceContextChangedCommandId = '_designerWorkspaceContext.didChange';

interface DesignerWorkspaceContextPreflight {
...
export class CommandCenter {
	...
	private static readonly designerBranchRemoteRefreshTimeoutMs = 2500;

	private disposables: Disposable[];
	private commandErrors = new CommandErrorOutputTextDocumentContentProvider();
	private readonly designerAutoSave: DesignerAutoSaveManager;
	...
	constructor(
		...
		this.designerAutoSave = new DesignerAutoSaveManager(this.model, this.globalState, this.logger);
		this.disposables = Commands.map(({ commandId, key, method, options }) => {
			const command = this.createCommand(commandId, key, method, options);
			return commands.registerCommand(commandId, command);
		});

		this.disposables.push(this.designerAutoSave);
	...
	async getDesignerBranchesState(options?: { updateRemotes?: boolean }): Promise<DesignerBranchState> {
		...
		if (options?.updateRemotes) {
			await this.tryUpdateDesignerBranchRefs(repository);
		}
	...
	private async checkoutDesignerBranchWithoutSaving(options?: { branchName?: string }): Promise<DesignerBranchCheckoutResult> {
		...
		const unsafeHostRepository = this.getDesignerHostRepositoryBlock(repository, state, branchName);
		if (unsafeHostRepository) {
			return unsafeHostRepository;
		}
		...
		const config = workspace.getConfiguration('git', Uri.file(repository.root));
		const pullBeforeCheckout = config.get<boolean>('pullBeforeCheckout', false) === true;
		const previousBranchName = repository.HEAD?.name;
		...
		if (!await this.confirmDesignerWorkspaceContextSwitch()) {
			return {
				state,
				blocked: {
					reason: 'switchFailed',
					message: l10n.t('Branch switch canceled.')
				}
			};
		}
	...
	async saveAndCheckoutDesignerBranch(options?: { branchName?: string }): Promise<DesignerBranchCheckoutResult> {
		...
		const repository = await this.pickDesignerRepository();
		await repository.status();
		const unsafeHostRepository = this.getDesignerHostRepositoryBlock(repository, await this.getDesignerBranchesStateForRepository(repository), branchName);
		if (unsafeHostRepository) {
			return unsafeHostRepository;
		}
	...
	private getDesignerHostRepositoryBlock(repository: Repository, state: DesignerBranchState, targetBranch?: string): DesignerBranchCheckoutResult | undefined {
		if (!isDesignerHostRepository(repository.root, __dirname)) {
			return undefined;
		}

		const message = l10n.t('This project is running the app. Open a separate project repo to switch branches safely.');
		return {
			state,
			blocked: {
			...
			sync: this.createDesignerBlockedSync('unsafeHostRepository', message, { previousBranch: repository.HEAD?.name, targetBranch })
		};
	}
	...
	@command('_designerBranches.saveBeforeCriticalAction')
	async saveBeforeDesignerCriticalAction(): Promise<DesignerSyncStatus> {
		const repository = await this.getDesignerWorkspaceRepository();
		if (!repository) {
			return { state: 'synced' };
		}
		...
		if (repository.mergeGroup.resourceStates.length > 0) {
			const message = l10n.t('Resolve merge conflicts before continuing.');
			return this.createDesignerBlockedSync('mergeConflicts', message, { previousBranch: repository.HEAD?.name });
		}

		const currentBranchName = await this.ensureDesignerCurrentBranch(repository);
		if (!currentBranchName) {
			const message = l10n.t('Add a branch name before continuing.');
			return this.createDesignerBlockedSync('branchNameRequired', message);
		}
	...
	@command('_designerBranches.pushCurrentSharedBranchWithConfirmation')
	async pushCurrentSharedDesignerBranchWithConfirmation(): Promise<DesignerSyncStatus> {
		const repository = await this.pickDesignerRepository();
		try {
			const pushed = await this.designerAutoSave.pushSharedBranchWithConfirmation(repository);
			return {
			...
			const blocked = this.getDesignerSaveBlocked(error);
			return this.createDesignerBlockedSync(blocked.reason, blocked.message, { previousBranch: repository.HEAD?.name });
		}
	}
	...
	private async confirmDesignerWorkspaceContextSwitch(): Promise<boolean> {
		let preflight: DesignerWorkspaceContextPreflight | undefined;
		try {
			preflight = await commands.executeCommand<DesignerWorkspaceContextPreflight>(DesignerWorkspaceContextPreflightCommandId);
		} catch (error) {
			this.logger.trace(`[DesignerBranches] Workspace context preflight unavailable: ${getDesignerErrorMessage(error)}`);
		}
		...
		const confirm = l10n.t('Switch Branch');
		const result = await window.showWarningMessage(preflight.message ?? l10n.t('Claude Code has active work in this window.'), { modal: true }, confirm);
		return result === confirm;
	}

	private async reconcileDesignerWorkspaceContext(repository: Repository, previousBranchName: string | undefined, reason: DesignerWorkspaceContextReason): Promise<void> {
		await repository.status();
		const branchName = repository.HEAD?.name;

		try {
			await commands.executeCommand(DesignerWorkspaceContextChangedCommandId, createDesignerWorkspaceContext({
			...
			this.logger.warn(`[DesignerBranches] Failed to request workspace context reconciliation: ${getDesignerErrorMessage(error)}`);
		}
	}

	private async saveDesignerCurrentBranch(repository: Repository, branchName: string): Promise<void> {
		await this.designerAutoSave.save(repository, 'manual', { branchName });
	...
	async saveAndSwitchDesignerRepo(options?: { repoPath?: string }): Promise<DesignerRepoSwitchResult> {
		...
		const targetHostRepositoryBlock = await this.getDesignerHostRepoSwitchBlock(repoPath);
		if (targetHostRepositoryBlock) {
			return targetHostRepositoryBlock;
		}
	...
	private async getDesignerHostRepoSwitchBlock(repoPath: string): Promise<DesignerRepoSwitchResult | undefined> {
		if (!isDesignerHostRepository(repoPath, __dirname)) {
			return undefined;
		}

		const message = l10n.t('This project is running the app. Open a separate project repo to switch safely.');
		return {
			state: await this.getDesignerReposStateInternal(),
			blocked: {
				reason: 'unsafeHostRepository',
			...
			sync: this.createDesignerBlockedSync('unsafeHostRepository', message, { targetRepoPath: repoPath })
		};
	}
	...
	private async getDesignerReposStateInternal(repoToInclude?: { path: string; name: string; url?: string }): Promise<DesignerRepoState> {
		...
		for (const repo of knownRepos) {
			if (isDesignerHostRepository(repo.path, __dirname)) {
				continue;
			}
	...
	private async storeDesignerKnownRepo(repoToStore: DesignerKnownRepo): Promise<void> {
		const currentRepoPath = workspace.workspaceFolders?.[0]?.uri.fsPath;
		const currentRepo = currentRepoPath && !isDesignerHostRepository(currentRepoPath, __dirname)
			? { path: currentRepoPath, name: getDesignerRepoLabel(currentRepoPath) }
			: undefined;
	...
	private async pickDesignerRepository(): Promise<Repository> {
		const workspaceRepository = await this.getDesignerWorkspaceRepository();
	...
	private async getDesignerWorkspaceRepository(): Promise<Repository | undefined> {
		const workspaceRoot = workspace.workspaceFolders?.[0]?.uri.fsPath;
		if (!workspaceRoot) {
			return undefined;
		}

		const exactRepository = findDesignerWorkspaceRepository(this.model.repositories, workspaceRoot);
		if (exactRepository) {
			return exactRepository;
		}

		try {
			const workspaceRootRealPath = await fs.promises.realpath(workspaceRoot);
			return findDesignerWorkspaceRepository(this.model.repositories, workspaceRoot, workspaceRootRealPath);
		} catch (error) {
			this.logger.trace(`[DesignerBranches] Failed to resolve workspace realpath: ${getDesignerErrorMessage(error)}`);
			return undefined;
		}
	}
	...
	private getDesignerBranchesSnapshotForRepository(repository: Repository, refs = repository.refs): DesignerBranchState {
		...
		const cloudProblem = this.designerAutoSave.getCloudProblem(repository.root, repository.HEAD?.name);
		const branches = mergeDesignerBranchRefs(branchRefs, defaultRemote?.name, defaultBranch, repository.HEAD?.name)
			.map(branch => branch.isCurrent && cloudProblem ? { ...branch, cloudProblem } : branch);
	...
	private installDesignerBranchStateBroadcaster(): Disposable {
		...
		const publish = (repository: Repository): void => {
			this.getDesignerWorkspaceRepository()
				.then(workspaceRepository => {
					if (repository !== workspaceRepository) {
						return;
					}

					return commands.executeCommand('_designerBranches.didChangeState', this.getDesignerBranchesSnapshotForRepository(repository));
				})
		...
		const addRepository = (repository: Repository): void => {
			...
			const repositoryDisposable = Disposable.from(
				repository.onDidRunGitStatus(() => publish(repository)),
				this.designerAutoSave.onDidChangeCloudProblems(() => publish(repository))
			);
	...
	private async tryUpdateDesignerBranchRefs(repository: Repository): Promise<boolean> {
		const refresh = this.updateDesignerBranchRefs(repository).then(
			() => true,
			error => {
				this.logger.trace(`[DesignerBranches] Failed to refresh remote branches: ${getDesignerErrorMessage(error)}`);
				return false;
			}
		);

		const timedRefresh = Promise.race([
			refresh,
			new Promise<'timeout'>(resolve => setTimeout(resolve, CommandCenter.designerBranchRemoteRefreshTimeoutMs, 'timeout'))
		]);

		const result = await timedRefresh;
		if (result === 'timeout') {
			this.logger.trace(`[DesignerBranches] Remote branch refresh timed out after ${CommandCenter.designerBranchRemoteRefreshTimeoutMs}ms; using local branch state.`);
			void refresh.then(success => {
				...
				return commands.executeCommand('_designerBranches.didChangeState', this.getDesignerBranchesSnapshotForRepository(repository));
			}, error => this.logger.trace(`[DesignerBranches] Failed to publish delayed branch refresh: ${getDesignerErrorMessage(error)}`));
			return false;
		}
		...
		for (const ref of refs) {
			...
			if (ref.type === RefType.Head) {
				branchRefs.push({
					...
					name: ref.name,
					commit: ref.commit,
					upstream: isBranch(ref) ? ref.upstream : undefined
				});
