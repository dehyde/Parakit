...
const tsgo = path.join(root, 'node_modules', '.bin', process.platform === 'win32' ? 'tsgo.cmd' : 'tsgo');
...
export function spawnTsgo(projectPath: string, config: { taskName: string; noEmit?: boolean }, onComplete?: () => Promise<void> | void): Promise<void> {
	...
	if (!fs.existsSync(tsgo)) {
		return Promise.reject(new Error(`tsgo was not found at ${tsgo}. Run npm install before building.`));
	}
	...
	const child = cp.spawn(tsgo, args.slice(1), {
		cwd: root,
