...
function isExtensionPackageEsm(configFilePath: string): boolean {
	const packageJsonPath = path.join(path.dirname(configFilePath), 'package.json');
	try {
		const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8')) as { type?: unknown };
...
export class ESBuildTranspiler implements ITranspiler {
	...
	constructor(
		...
		const isEsmExtension = isExtension && isExtensionPackageEsm(configFilePath);

		const target = getTargetStringFromTsConfig(configFilePath);

		this._transformOpts = {
			target: [target],
			format: isExtension && !isEsmExtension ? 'cjs' : 'esm',
			...
			tsconfigRaw: JSON.stringify({
				compilerOptions: {
					...
					...{
						module: isExtension && !isEsmExtension ? ts.ModuleKind.CommonJS : undefined
					} satisfies ts.CompilerOptions
				}
			}),
			supported: {
				'class-static-blocks': false, // SEE https://github.com/evanw/esbuild/issues/3823,
				'dynamic-import': !isExtension || isEsmExtension, // see https://github.com/evanw/esbuild/issues/1281
				'class-field': !isExtension || isEsmExtension
			}
		};
	}
