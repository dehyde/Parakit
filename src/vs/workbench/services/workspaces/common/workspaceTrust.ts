...
import { getDesignerManagedRepoForFolder, readDesignerManagedReposManifest } from './designerManagedRepos.js';
...
export class WorkspaceTrustManagementService extends Disposable implements IWorkspaceTrustManagementService {
	...
	private initializeWorkspaceTrust(): void {
		...
		this.resolveCanonicalUris()
			.then(async () => {
				...
				await this.seedDesignerManagedWorkspaceTrust();
	...
	private async seedDesignerManagedWorkspaceTrust(): Promise<void> {
		if (!this.workspaceTrustEnablementService.isWorkspaceTrustEnabled() || this.environmentService.remoteAuthority) {
			return;
		}

		const workspaceFolders = this._canonicalWorkspace.folders.map(folder => folder.uri);
		if (workspaceFolders.length === 0 || workspaceFolders.some(folder => folder.scheme !== Schemas.file)) {
			return;
		}
		...
		const manifest = await readDesignerManagedReposManifest(this.fileService, nativeEnvironment.userHome);
		const trustedRepoUris: URI[] = [];
		for (const folder of workspaceFolders) {
			const managedRepo = getDesignerManagedRepoForFolder(manifest, folder.fsPath, nativeEnvironment.appRoot);
			if (!managedRepo) {
				return;
			}

			trustedRepoUris.push(URI.file(managedRepo.path));
		}

		await this.doSetUrisTrust(trustedRepoUris, true);
	}
