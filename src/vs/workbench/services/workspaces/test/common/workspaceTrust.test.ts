...
import { getDesignerManagedReposManifestResource } from '../../common/designerManagedRepos.js';
...
suite('Workspace Trust', () => {
	...
	suite('Management', () => {
		...
		test('designer-managed repo starts trusted from manifest', async () => {
			setDesignerNativeEnvironment('/Users/test', '/Users/test/Documents/VSCode Fork');
			await writeDesignerManagedManifest({
				version: 1,
				repos: [{ path: '/Users/test/Documents/Designer Repos/app', name: 'app', addedAt: 1 }]
			});

			workspaceService.setWorkspace(testWorkspace(URI.file('/Users/test/Documents/Designer Repos/app')));
			const testObject = await initializeTestObject();

			assert.strictEqual(testObject.isWorkspaceTrusted(), true);
		});

		test('unmanaged repo keeps normal restricted mode behavior', async () => {
			setDesignerNativeEnvironment('/Users/test', '/Users/test/Documents/VSCode Fork');
			await writeDesignerManagedManifest({
				version: 1,
				repos: [{ path: '/Users/test/Documents/Designer Repos/app', name: 'app', addedAt: 1 }]
			...
			workspaceService.setWorkspace(testWorkspace(URI.file('/Users/test/Documents/Other Repo')));
			const testObject = await initializeTestObject();

			assert.strictEqual(testObject.isWorkspaceTrusted(), false);
		});

		test('child folder inside designer-managed repo starts trusted', async () => {
			setDesignerNativeEnvironment('/Users/test', '/Users/test/Documents/VSCode Fork');
			await writeDesignerManagedManifest({
				version: 1,
				repos: [{ path: '/Users/test/Documents/Designer Repos/app', name: 'app', addedAt: 1 }]
			...
			workspaceService.setWorkspace(testWorkspace(URI.file('/Users/test/Documents/Designer Repos/app/packages/site')));
			const testObject = await initializeTestObject();

			assert.strictEqual(testObject.isWorkspaceTrusted(), true);
		});

		test('multi-root workspace starts trusted only when every folder is designer-managed', async () => {
			setDesignerNativeEnvironment('/Users/test', '/Users/test/Documents/VSCode Fork');
			await writeDesignerManagedManifest({
				version: 1,
				repos: [
					{ path: '/Users/test/Documents/Designer Repos/app-a', name: 'app-a', addedAt: 1 },
					{ path: '/Users/test/Documents/Designer Repos/app-b', name: 'app-b', addedAt: 2 }
				]
			});

			workspaceService.setWorkspace(testWorkspace(
				URI.file('/Users/test/Documents/Designer Repos/app-a'),
				URI.file('/Users/test/Documents/Designer Repos/app-b/packages/site')
			));
			const trustedObject = await initializeTestObject();

			assert.strictEqual(trustedObject.isWorkspaceTrusted(), true);

			storageService = store.add(new TestStorageService());
			instantiationService.stub(IStorageService, storageService);
			workspaceService.setWorkspace(testWorkspace(
				URI.file('/Users/test/Documents/Designer Repos/app-a'),
				URI.file('/Users/test/Documents/Other Repo')
			));
			const untrustedObject = await initializeTestObject();

			assert.strictEqual(untrustedObject.isWorkspaceTrusted(), false);
		});

		test('invalid designer-managed manifest is ignored safely', async () => {
			setDesignerNativeEnvironment('/Users/test', '/Users/test/Documents/VSCode Fork');
			const manifestResource = getDesignerManagedReposManifestResource(getDesignerUserHome());
			await fileService.createFolder(dirname(manifestResource));
			await fileService.writeFile(manifestResource, VSBuffer.fromString('{'));
		...
		test('app source repo is not trusted from designer manifest', async () => {
			setDesignerNativeEnvironment('/Users/test', '/Users/test/Documents/VSCode Fork');
			await writeDesignerManagedManifest({
				version: 1,
			...
			workspaceService.setWorkspace(testWorkspace(URI.file('/Users/test/Documents/VSCode Fork')));
			const testObject = await initializeTestObject();
		...
		async function writeDesignerManagedManifest(manifest: object): Promise<void> {
			const manifestResource = getDesignerManagedReposManifestResource(getDesignerUserHome());
			await fileService.createFolder(dirname(manifestResource));
			await fileService.writeFile(manifestResource, VSBuffer.fromString(JSON.stringify(manifest)));
		}

		function setDesignerNativeEnvironment(userHomePath: string, appRoot: string): void {
			environmentService = {
				...environmentService,
				userHome: URI.file(userHomePath),
				appRoot
			} as Mutable<IWorkbenchEnvironmentService>;
			instantiationService.stub(IWorkbenchEnvironmentService, environmentService);
		}

		function getDesignerUserHome(): URI {
			return (environmentService as IWorkbenchEnvironmentService & { readonly userHome: URI }).userHome;
		}
	});
