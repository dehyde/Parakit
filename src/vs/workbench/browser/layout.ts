...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	protected restoreParts(): void {
		...
		layoutReadyPromises.push((async () => {
			...
			if (!this.state.initialization.views.containerToRestore.sideBar || !this.isVisible(Parts.SIDEBAR_PART)) {
				return;
			}
	...
	private setActivityBarHidden(hidden: boolean): void {
		...
		const activityBar = this.getPart(Parts.ACTIVITYBAR_PART) as ActivitybarPart;
		if (hidden) {
			activityBar.hide();
		} else {
			activityBar.show();
...
export function forcePrimarySidebarClosedOnStartup(stateCache: Map<string, unknown>): void {
	stateCache.set(LayoutStateKeys.ACTIVITYBAR_HIDDEN.name, true);
	stateCache.set(LayoutStateKeys.SIDEBAR_HIDDEN.name, true);
}
...
class LayoutStateModel extends Disposable {
	...
	load(configuration: ILayoutStateLoadConfiguration): void {
		...
		forcePrimarySidebarClosedOnStartup(this.stateCache);
	...
	private isActivityBarHidden(): boolean {
		...
		return (activityBarLocation.value ?? activityBarLocation.defaultValue) !== ActivityBarPosition.DEFAULT;
	}
