...
export class MultiEditorTabsControl extends EditorTabsControl {
	...
	private redrawTab(editor: EditorInput, tabIndex: number, tabContainer: HTMLElement, tabLabelWidget: IResourceLabel, tabLabel: IEditorInputLabel, tabActionBar: ActionBar): void {
		...
		const customTabAction = editor.getTabAction();
		const hasCustomTabAction = !!customTabAction;
		const hasAction = hasCustomTabAction || hasUnpinAction || hasCloseAction;
		...
		tabContainer.classList.toggle(`close-action-off`, !hasCustomTabAction && !hasUnpinAction && !hasCloseAction);
