...
import './media/webviewViewPane.css';

const storageKeys = {
	webviewState: 'webviewState',
} as const;

const claudeCodeViewIds = new Set([
	'claudeVSCodeSidebar',
...
export class WebviewViewPane extends ViewPane {
	...
	private _claudeCodeLoadingComplete = false;
	...
	private createClaudeCodeLoadingOverlay(container: HTMLElement): HTMLElement {
		const overlay = document.createElement('div');
		overlay.className = 'claude-code-sidebar-loading';
		overlay.setAttribute('role', 'status');
		overlay.setAttribute('aria-live', 'polite');

		const mark = document.createElement('div');
		mark.className = 'claude-code-sidebar-loading-mark';

		const ring = document.createElement('div');
		ring.className = 'claude-code-sidebar-loading-ring';
		mark.appendChild(ring);

		const dots = document.createElement('div');
		dots.className = 'claude-code-sidebar-loading-dots';
		for (let i = 0; i < 3; i++) {
			dots.appendChild(document.createElement('span'));
		}

		const label = document.createElement('div');
		label.className = 'claude-code-sidebar-loading-label';
		label.textContent = 'Claude Code is loading';

		const detail = document.createElement('div');
		detail.className = 'claude-code-sidebar-loading-detail';
		detail.textContent = 'Preparing the agent panel';

		overlay.append(mark, label, dots, detail);
		container.appendChild(overlay);
		return overlay;
	}

	private showClaudeCodeLoading(webview: IOverlayWebview): void {
		if (!this.isClaudeCodeWebviewView() || this._claudeCodeLoadingComplete) {
			return;
		}

		const container = webview.container;
		container.classList.add('claude-code-sidebar-webview-container');
		this._loadingOverlay?.remove();
		this._loadingOverlay = this.createClaudeCodeLoadingOverlay(container);
		...
		const hideOverlay = () => {
			...
			loadingDone = true;
			this._claudeCodeLoadingComplete = true;
			container.classList.add('claude-code-sidebar-webview-loaded');
			this._loadingOverlay?.classList.add('claude-code-sidebar-loading-hidden');
		};
		this._webviewDisposables.add(webview.onDidLoad(() => hideOverlay()));
		this._webviewDisposables.add(webview.onMessage(() => hideOverlay()));
		this._webviewDisposables.add(webview.onFatalError(() => hideOverlay()));
		this._webviewDisposables.add(toDisposable(() => {
			this._loadingOverlay?.remove();
			this._loadingOverlay = undefined;
			container.classList.remove('claude-code-sidebar-webview-container', 'claude-code-sidebar-webview-loaded');
		}));
	}

	private isClaudeCodeWebviewView(): boolean {
		return claudeCodeViewIds.has(this.id);
	}
	...
	private updateTreeVisibility() {
		if (this.isBodyVisible()) {
			...
			this._webview.value && this.showClaudeCodeLoading(this._webview.value);
		} else {
