...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate Parakit and choose when to capture.", MAX_ATTACHMENTS);
	...
	private createFloatingCaptureBar(): void {
		...
		// Mount inside .monaco-workbench so Parakit's color theme CSS vars
		...
		// Delay/options dropdown using Parakit's context menu
	...
	private getSourceOptions(): { label: string; value: IssueSource }[] {
		const options: { label: string; value: IssueSource }[] = [
			...
			{ label: localize('extensionSource', "A Parakit extension"), value: IssueSource.Extension },
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			...
			case IssueSource.Extension:
				return this.selectedExtension?.displayName || this.selectedExtension?.name || localize('extensionSource', "A Parakit extension");
			case IssueSource.Marketplace:
	...
	private updateReviewDetails(): void {
			...
			this.createDiagSection(diagContainer, {
				...
				renderContent: (container) => {
					...
					if (modelData.versionInfo) {
						this.addDiagRow(sysTable, 'Parakit', modelData.versionInfo.vscodeVersion);
		...
		if (modelData.extensionData) {
			...
			// Extension (e.g. built-in extensions are filed against Parakit),
	...
	private generateIssueDetailsMd(): string {
		...
		const rows: [string, string | undefined][] = [
			...
			['Parakit Version', modelData.versionInfo?.vscodeVersion ?? product.version],
	...
	private generateSystemInfoMd(): string {
		...
		if (modelData.versionInfo) {
			rows.push(
				['Parakit Version', modelData.versionInfo.vscodeVersion],
