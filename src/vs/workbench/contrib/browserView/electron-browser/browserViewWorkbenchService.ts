...
import { BrowserViewCommandId, BrowserViewKind, BrowserViewStorageScope, getBrowserViewKindForInitialState, IBrowserViewOpenOptions, IBrowserViewOwner, IBrowserViewService, IBrowserViewState, IBrowserViewTheme, ipcBrowserViewChannelName } from '../../../../platform/browserView/common/browserView.js';
...
export class BrowserViewWorkbenchService extends Disposable implements IBrowserViewWorkbenchService {
	...
	getOrCreateLazy(id: string, initialState?: IBrowserEditorViewState, options?: { readonly kind?: BrowserViewKind }, model?: IBrowserViewModel): BrowserEditorInput {
		if (!this._known.has(id)) {
			const owner = this._getDefaultOwner(options?.kind ?? getBrowserViewKindForInitialState(initialState));
			const input = this.instantiationService.createInstance(BrowserEditorInput, { id, ...initialState }, async () => {
				...
				return this._createModel(id, owner, state);
			});
			input.onWillDispose(() => {
	...
	private _getDefaultOwner(kind?: BrowserViewKind): IBrowserViewOwner {
		return { mainWindowId: this._mainWindowId, kind: kind ?? BrowserViewKind.Browser };
	}
