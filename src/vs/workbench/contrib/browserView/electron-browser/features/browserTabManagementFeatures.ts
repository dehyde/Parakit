...
import { BrowserViewCommandId, getBrowserViewExternalLinkAction } from '../../../../../platform/browserView/common/browserView.js';
...
class LocalhostLinkOpenerContribution extends Disposable implements IWorkbenchContribution, IExternalOpener {
	...
	async openExternal(href: string, ctx: { sourceUri: URI; preferredOpenerId?: string }, _token: CancellationToken): Promise<boolean> {
		...
		let isLocalDevUrl = false;
		try {
			const parsed = new URL(href);
			if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
				return false;
			}
			isLocalDevUrl = isLocalhostAuthority(parsed.host) || isAllInterfacesAuthority(parsed.host);
			if (isLocalDevUrl) {
		...
		const action = getBrowserViewExternalLinkAction({
			targetUrl: href,
			hasAppPreview: Boolean(appPreview),
			openLocalhostLinks: this.configurationService.getValue<boolean>('workbench.browser.openLocalhostLinks'),
		});

		if (action === 'openAppPreview' && appPreview) {
			appPreview.navigate(appPreviewHref ?? href);
			await this.editorService.openEditor(appPreview, { pinned: true, index: 0 });
			return true;
		}

		if (action === 'allowExternal') {
			return false;
		}
		...
		const isDefaultLinkOpen = isLocalDevUrl && !isConfigured(this.configurationService.inspect('workbench.browser.openLocalhostLinks'));
