...
import { IBrowserDesignElementService } from '../../common/browserDesignElementService.js';
...
export function createElementContextValue(elementData: IElementData, displayName: string): string {
...
export async function isTrustedContentAttachmentUrl(url: string, trustFileUri?: (uri: URI) => Promise<boolean>): Promise<boolean> {
	try {
		const parsedUrl = new URL(url);
		if (parsedUrl.protocol === 'file:') {
			return await trustFileUri?.(URI.file(parsedUrl.pathname)) ?? false;
		}
...
export class BrowserEditorChatIntegration extends BrowserEditorContribution {
	...
	private readonly _elementSelectionWarning: HTMLElement;
	private readonly _hideElementSelectionWarningScheduler = this._register(new RunOnceScheduler(() => {
	...
	constructor(
		...
		@IBrowserDesignElementService private readonly browserDesignElementService: IBrowserDesignElementService,
	) {
		super(editor);
		...
		this._shareButtonContainer.style.display = 'none';
		...
		this._elementSelectionWarning = document.createElement('div');
		this._elementSelectionWarning.className = 'browser-element-selection-warning';
		this._elementSelectionWarning.textContent = localize('browser.elementSelectionWarning', "Use caution when selecting content from untrusted sources. Pages may contain hidden prompts.");
		this._elementSelectionWarning.setAttribute('role', 'status');
		this._elementSelectionWarning.style.display = 'none';
	...
	override get widgets(): readonly IBrowserEditorWidget[] {
		return [
			{ location: BrowserWidgetLocation.PostUrl, element: this._shareButtonContainer, order: 100 },
			{ location: BrowserWidgetLocation.ContentArea, element: this._elementSelectionWarning, order: 400 }
		];
	}
	...
	protected override onModelAttached(model: IBrowserViewModel, store: DisposableStore): void {
		...
		store.add(model.onDidSelectElement(async data => {
			try {
				await this._inspectElementData(data, model);
			} catch (error) {
				this.logService.error('BrowserEditor.addElementToChat: Failed to inspect element', error);
			}
		}));
		...
		store.add(model.onDidChangeElementSelectionActive(active => {
			...
			void this._updateElementSelectionWarning(active, model);
		}));
		void this._updateElementSelectionWarning(model.isElementSelectionActive, model);
	...
	override onModelDetached(): void {
		...
		this._hideElementSelectionWarningScheduler.cancel();
		this._setElementSelectionWarningVisible(false);
	}
	...
	private _updateSharingState(isInitialState: boolean): void {
		...
		const presentation = getBrowserShareControlPresentation(model?.sharingState);
		...
		this._shareButtonContainer.style.display = presentation.visible ? '' : 'none';
		this._shareButton.checked = presentation.checked;
		this._shareButton.label = presentation.label;
		this._shareButton.element.classList.toggle('stop-ai-control', presentation.isStopControl);
		this._shareButton.setTitle(presentation.title);
		this._shareButton.element.setAttribute('aria-label', presentation.ariaLabel);
	}
	...
	private async _confirmContentAttachmentRisk(url: string): Promise<boolean> {
		...
		if (await isTrustedContentAttachmentUrl(url, async uri => {
	...
	private async _updateElementSelectionWarning(active: boolean, model: IBrowserViewModel): Promise<void> {
		this._hideElementSelectionWarningScheduler.cancel();
		if (!active) {
			this._setElementSelectionWarningVisible(false);
			return;
		...
		const trusted = await isTrustedContentAttachmentUrl(model.url, async uri => {
			const trustInfo = await this.workspaceTrustManagementService.getUriTrustInfo(uri);
			return trustInfo.trusted;
		});

		if (model !== this.editor.model || !model.isElementSelectionActive) {
			return;
		}

		this._setElementSelectionWarningVisible(!trusted);
	}

	private _setElementSelectionWarningVisible(visible: boolean): void {
		this._elementSelectionWarning.style.display = visible ? '' : 'none';
	}
	...
	private async _inspectElementData(elementData: IElementData, model: IBrowserViewModel) {
		...
		type IntegratedBrowserAddElementToChatInspectedEvent = {};

		type IntegratedBrowserAddElementToChatInspectedClassification = {
			owner: 'jruales';
			comment: 'An element context was successfully inspected from Integrated Browser.';
		};

		this.telemetryService.publicLog2<IntegratedBrowserAddElementToChatInspectedEvent, IntegratedBrowserAddElementToChatInspectedClassification>('integratedBrowser.addElementToChat.inspected', {});
	}
