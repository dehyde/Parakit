...
class GoHomeAction extends Action2 {
	static readonly ID = NavigateWorkbenchAppPreviewHomeCommandId;

	constructor() {
		super({
			id: GoHomeAction.ID,
			title: localize2('browser.goHomeAction', 'Go Home'),
			category: BrowserActionCategory,
			icon: Codicon.home,
			f1: true,
			precondition: BROWSER_EDITOR_ACTIVE,
			menu: {
				...
				order: 4,
				alt: {
					id: PickWorkbenchAppPreviewHomeCommandId,
					title: localize2('browser.chooseHomeAction', 'Choose Home URL'),
					icon: Codicon.home,
				}
			}
		});
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		await commandService.executeCommand(NavigateWorkbenchAppPreviewHomeCommandId);
	}
}
