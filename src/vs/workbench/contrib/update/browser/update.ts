...
export async function saveDesignerChangesBeforeUpdate(commandService: ICommandService): Promise<void> {
	const result = await commandService.executeCommand<{ state?: string; message?: string }>('_designerBranches.saveBeforeCriticalAction');
	if (result?.state === 'blocked' || result?.state === 'problem') {
		throw new Error(result.message ?? nls.localize('designerAutoSaveBeforeUpdateFailed', 'Save changes before updating, then try again.'));
	}
}
...
export class UpdateContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerGlobalActivityActions(): void {
		...
		CommandsRegistry.registerCommand('update.install', async () => {
			await this.saveDesignerChangesBeforeUpdate();
	...
	private async saveDesignerChangesBeforeUpdate(): Promise<void> {
		await saveDesignerChangesBeforeUpdate(this.commandService);
	}
}
			...
			this._register(registerAction2(class SwitchQuality extends Action2 {
					...
					try {
						...
						const res = await dialogService.confirm({
							...
							detail: newQuality === 'insider' ?
								nls.localize('relaunchDetailInsiders', "Press the reload button to switch to the Insiders version of Parakit.") :
								nls.localize('relaunchDetailStable', "Press the reload button to switch to the Stable version of Parakit."),
				...
				private async selectSettingsSyncService(dialogService: IDialogService): Promise<UserDataSyncStoreType | undefined> {
					const { result } = await dialogService.prompt<UserDataSyncStoreType>({
						...
						detail: nls.localize('selectSyncService.detail', "The Insiders version of Parakit will synchronize your settings, keybindings, extensions, snippets and UI State using separate insiders settings sync service by default."),
						buttons: [
							{
