...
import { ProductContribution, UpdateContribution, CONTEXT_UPDATE_STATE, SwitchProductQualityContribution, showReleaseNotesInEditor, DefaultAccountUpdateContribution, saveDesignerChangesBeforeUpdate } from './update.js';
...
class InstallUpdateAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		await saveDesignerChangesBeforeUpdate(accessor.get(ICommandService));
