...
import { getStableActionWidgetAnchor } from '../../browser/actionWidget.js';
...
suite('ActionListWidget', () => {
	...
	test('stable action widget anchor keeps the original element position after the element is detached', () => {
		...
		const stableAnchor = getStableActionWidgetAnchor(anchor);
		anchor.remove();

		assert.deepStrictEqual(stableAnchor, { x: 12, y: 34, width: 100, height: 20 });
	});
