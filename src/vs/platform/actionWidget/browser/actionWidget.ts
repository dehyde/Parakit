...
export function getStableActionWidgetAnchor(anchor: HTMLElement): IAnchor {
	const targetWindow = dom.getWindow(anchor);
	const rect = anchor.getBoundingClientRect();
	return {
		x: rect.left + targetWindow.pageXOffset,
		y: rect.top + targetWindow.pageYOffset,
		width: rect.width,
		height: rect.height,
	};
}
...
class ActionWidgetService extends Disposable implements IActionWidgetService {
	...
	private _renderWidget(element: HTMLElement, list: ActionList<unknown>, actionBarActions: readonly IAction[]): IDisposable {
		...
		renderDisposables.add(dom.addDisposableListener(targetWindow, dom.EventType.MOUSE_DOWN, (e: MouseEvent) => {
			...
			if (target?.closest('.action-widget') || target?.closest('.action-widget-hover') || target?.closest('.action-list-submenu-panel')) {
				return;
			}
