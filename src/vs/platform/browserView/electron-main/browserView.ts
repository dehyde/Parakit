...
export class BrowserView extends Disposable {
	...
	constructor(
		...
		this._view.webContents.setWindowOpenHandler((details) => {
			...
			if (shouldOpenBrowserViewTargetInternally(details.url)) {
				return {
					action: 'allow',
					createWindow: (options) => this._createChildView(details.url, options, {
						pinned: true,
	...
	private _returnAppPreviewAuthCallback(url: string): boolean {
		if (!this._appPreviewAuthReturnView || this._appPreviewAuthCallbackHandled) {
			return false;
		}
	...
	private setupEventListeners(): void {
		...
		webContents.on('page-favicon-updated', async (_event, favicons) => {
			...
			webContents.on('will-navigate', (event) => {
				const authAction = getBrowserViewAuthNavigationAction({
					kind: this.owner.kind,
					currentUrl: this.webContents.getURL(),
					targetUrl: event.url,
					inAuthWindow: !!this._appPreviewAuthReturnView
				...
				if (authAction === 'returnToPreview' && this._appPreviewAuthReturnView) {
					...
					this._returnAppPreviewAuthCallback(event.url);
					return;
				}
				if (authAction === 'openInternal') {
					event.preventDefault();
					this._openAppPreviewInternalNavigation(event.url);
					return;
				}
