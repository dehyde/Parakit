...
export interface IElementMatchedStyleRule {
	...
	readonly properties: readonly IElementMatchedStyleProperty[];
	readonly styleSheetId?: string;
	readonly sourceURL?: string;
	readonly startLine?: number;
}

export interface IElementMatchedStyleProperty {
...
export interface IBrowserViewWindowConfiguration {
	 ...
	 * Usually `getTrustedUris()` plus the active workspace folder paths, so
	 * configured local previews can open files that belong to the current repo.
	 */
...
export interface IBrowserViewOwner {
	...
	/** Semantic kind used to scope specialized browser-view behavior. */
	readonly kind?: BrowserViewKind;
}

export const enum BrowserViewKind {
	Browser = 'browser',
	AppPreview = 'appPreview'
}

export interface IBrowserViewAuthNavigationRequest {
	readonly kind: BrowserViewKind | undefined;
...
export interface IBrowserViewExternalLinkRequest {
	readonly targetUrl: string;
	readonly hasAppPreview: boolean;
	readonly openLocalhostLinks: boolean;
}

export type BrowserViewExternalLinkAction = 'allowExternal' | 'openAppPreview' | 'openInternal';

export function getBrowserViewKindForInitialState(initialState: IBrowserViewKindInitialState | undefined): BrowserViewKind {
	return initialState?.isSessionAppPreview ? BrowserViewKind.AppPreview : BrowserViewKind.Browser;
}

export function isBrowserViewLocalHttpUrl(url: string | undefined): boolean {
	const parsed = parseBrowserViewWebUrl(url);
	if (!parsed) {
		return false;
	}
...
function isBrowserViewLocalOrAllInterfacesHttpUrl(url: string | undefined): boolean {
	const parsed = parseBrowserViewWebUrl(url);
	if (!parsed) {
		return false;
	}
...
function parseBrowserViewWebUrl(url: string | undefined): URL | undefined {
	if (!url) {
		return undefined;
	}
...
function isBrowserViewWebUrl(url: string): boolean {
	return Boolean(parseBrowserViewWebUrl(url));
}

export function shouldOpenBrowserViewTargetInternally(url: string): boolean {
	return isBrowserViewWebUrl(url);
}

export function getBrowserViewExternalLinkAction(request: IBrowserViewExternalLinkRequest): BrowserViewExternalLinkAction {
	if (!isBrowserViewWebUrl(request.targetUrl)) {
		return 'allowExternal';
	}

	if (isBrowserViewLocalOrAllInterfacesHttpUrl(request.targetUrl)) {
		return request.hasAppPreview ? 'openAppPreview' : request.openLocalhostLinks ? 'openInternal' : 'allowExternal';
	}

	return request.hasAppPreview ? 'openInternal' : 'allowExternal';
}

export function getBrowserViewAuthNavigationAction(request: IBrowserViewAuthNavigationRequest): BrowserViewAuthNavigationAction {
	if (request.kind !== BrowserViewKind.AppPreview || !isBrowserViewWebUrl(request.targetUrl)) {
		return 'allow';
	}
