...
function getHorizontalGap(a: DOMRect, b: DOMRect): { readonly from: number; readonly to: number } | undefined {
	if (a.right <= b.left) {
		return { from: a.right, to: b.left };
	}
	if (b.right <= a.left) {
		return { from: b.right, to: a.left };
	}
	return undefined;
}

function getVerticalGap(a: DOMRect, b: DOMRect): { readonly from: number; readonly to: number } | undefined {
	if (a.bottom <= b.top) {
		return { from: a.bottom, to: b.top };
	}
	if (b.bottom <= a.top) {
		return { from: b.bottom, to: a.top };
	}
	return undefined;
}

function getOverlapCenter(aStart: number, aEnd: number, bStart: number, bEnd: number): number | undefined {
...
class ElementPicker {
	...
	private _measurementActive = false;
	...
	private readonly _pinnedHighlight: HTMLDivElement;
	private readonly _measurementLayer: HTMLDivElement;
	...
	private _pinnedTarget: Element | undefined;
	...
	constructor(
		...
		const pinnedHighlight = document.createElement('div');
		pinnedHighlight.className = 'pinned-highlight';
		pinnedHighlight.style.display = 'none';
		root.appendChild(pinnedHighlight);
		this._pinnedHighlight = pinnedHighlight;

		const measurementLayer = document.createElement('div');
		measurementLayer.className = 'measurement-layer';
		root.appendChild(measurementLayer);
	...
	private _onPointerDown = (e: PointerEvent): void => {
		...
		if (this._measurementActive) {
			const target = this._pickElementAt(e.clientX, e.clientY);
			if (target) {
				this._commit(target);
			}
			e.preventDefault();
	...
	private _onScrollOrResize(): void {
		if (this._highlightTarget) {
			...
			this._renderMeasurements(this._highlightTarget);
		}
		if (this._pinnedTarget) {
			this._renderPinnedHighlight();
		}
	}
	...
	private _renderHighlight(target: Element): void {
		...
		this._renderElementBox(highlight, rect, scrollX, scrollY);
		...
		this._labelDims.textContent = `${Math.round(rect.width)} x ${Math.round(rect.height)}${this._formatPinnedComparison(target, rect)}`;
	...
	private _updateHighlight(target: Element | undefined): void {
		...
		this._renderMeasurements(target);
	}

	private _renderPinnedHighlight(): void {
		if (!this._pinnedTarget) {
			this._pinnedHighlight.style.display = 'none';
			return;
		}
		...
		this._renderElementBox(this._pinnedHighlight, rect, window.scrollX || 0, window.scrollY || 0);
	}

	private _renderElementBox(element: HTMLElement, rect: DOMRect, scrollX: number, scrollY: number): void {
	...
	private _formatPinnedComparison(target: Element, rect: DOMRect): string {
		if (!this._pinnedTarget || this._pinnedTarget === target) {
			return '';
		}

		const pinned = this._pinnedTarget.getBoundingClientRect();
		const hGap = pinned.right <= rect.left
			? Math.round(rect.left - pinned.right)
			: rect.right <= pinned.left
				? Math.round(pinned.left - rect.right)
				: 0;
		const vGap = pinned.bottom <= rect.top
			? Math.round(rect.top - pinned.bottom)
			: rect.bottom <= pinned.top
				? Math.round(pinned.top - rect.bottom)
				: 0;
		const aligned: string[] = [];
		if (Math.round(pinned.left) === Math.round(rect.left)) {
			aligned.push('left');
		}
		if (Math.round(pinned.right) === Math.round(rect.right)) {
			aligned.push('right');
		}
		if (Math.round(pinned.top) === Math.round(rect.top)) {
			aligned.push('top');
		}
		if (Math.round(pinned.bottom) === Math.round(rect.bottom)) {
			aligned.push('bottom');
		}

		const alignment = aligned.length ? `, align ${aligned.join('/')}` : '';
		return `, h ${hGap}px, v ${vGap}px${alignment}`;
	}

	private _renderMeasurements(target: Element): void {
		this._clearMeasurements();
		if (!this._pinnedTarget || this._pinnedTarget === target) {
			return;
		}
		...
		const pinned = this._pinnedTarget.getBoundingClientRect();
		const hovered = target.getBoundingClientRect();
		const hGap = getHorizontalGap(pinned, hovered);
		const vGap = getVerticalGap(pinned, hovered);

		if (hGap) {
			const y = getOverlapCenter(pinned.top, pinned.bottom, hovered.top, hovered.bottom) ?? ((pinned.top + pinned.bottom + hovered.top + hovered.bottom) / 4);
			this._appendMeasurementLine('horizontal', hGap.from, y, hGap.to - hGap.from, `${Math.round(hGap.to - hGap.from)}px`);
		}

		if (vGap) {
			const x = getOverlapCenter(pinned.left, pinned.right, hovered.left, hovered.right) ?? ((pinned.left + pinned.right + hovered.left + hovered.right) / 4);
			this._appendMeasurementLine('vertical', x, vGap.from, vGap.to - vGap.from, `${Math.round(vGap.to - vGap.from)}px`);
		}

		this._appendAlignmentGuide('left', pinned.left, hovered.left, Math.min(pinned.top, hovered.top), Math.max(pinned.bottom, hovered.bottom));
		this._appendAlignmentGuide('right', pinned.right, hovered.right, Math.min(pinned.top, hovered.top), Math.max(pinned.bottom, hovered.bottom));
		this._appendAlignmentGuide('top', pinned.top, hovered.top, Math.min(pinned.left, hovered.left), Math.max(pinned.right, hovered.right));
		this._appendAlignmentGuide('bottom', pinned.bottom, hovered.bottom, Math.min(pinned.left, hovered.left), Math.max(pinned.right, hovered.right));
	}

	private _appendMeasurementLine(direction: 'horizontal' | 'vertical', x: number, y: number, length: number, label: string): void {
		const line = document.createElement('div');
		line.className = `measurement-line ${direction}`;
		...
		this._measurementLayer.appendChild(line);

		const badge = document.createElement('div');
		badge.className = 'measurement-badge';
		badge.textContent = label;
		badge.style.left = direction === 'horizontal' ? `${x + length / 2}px` : `${x}px`;
		badge.style.top = direction === 'horizontal' ? `${y}px` : `${y + length / 2}px`;
		this._measurementLayer.appendChild(badge);
	}

	private _appendAlignmentGuide(edge: 'left' | 'right' | 'top' | 'bottom', pinnedValue: number, hoveredValue: number, from: number, to: number): void {
		if (Math.round(pinnedValue) !== Math.round(hoveredValue)) {
			return;
		}

		const guide = document.createElement('div');
		guide.className = `alignment-guide ${edge === 'left' || edge === 'right' ? 'vertical' : 'horizontal'}`;
		if (edge === 'left' || edge === 'right') {
			guide.style.left = `${pinnedValue}px`;
			guide.style.top = `${from}px`;
			guide.style.height = `${to - from}px`;
		} else {
			guide.style.left = `${from}px`;
			guide.style.top = `${pinnedValue}px`;
			guide.style.width = `${to - from}px`;
		}
		this._measurementLayer.appendChild(guide);
	}

	private _clearMeasurements(): void {
		this._measurementLayer.replaceChildren();
	}
	...
	private static _buildStyle(): HTMLStyleElement {
		...
		style.textContent = `
			...
			.pinned-highlight {
				position: absolute; box-sizing: border-box;
				border: 2px solid var(--vscode-button-background, #0078d4);
				background: color-mix(in srgb, var(--vscode-button-background, #0078d4) 16%, transparent);
				border-radius: 2px;
			...
			.alignment-guide.horizontal {
				border-top: 1px dashed var(--vscode-button-background, #0078d4);
			}
			.alignment-guide.vertical {
				border-left: 1px dashed var(--vscode-button-background, #0078d4);
			}
			.overlay {
				position: fixed; inset: 0;
