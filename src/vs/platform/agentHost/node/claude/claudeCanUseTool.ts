...
async function dispatchCanUseTool(
	...
	if (isToolDenied(deps, session, toolName)) {
		return { behavior: 'deny', message: `Tool '${toolName}' is disabled in this environment.` };
	}
...
function isToolDenied(deps: IClaudeCanUseToolDeps, session: ClaudeAgentSession, toolName: string): boolean {
	const permissions = deps.configurationService.getEffectiveValue(session.sessionUri.toString(), platformSessionSchema, SessionConfigKey.Permissions);
	return permissions?.deny.some(pattern => matchesToolPattern(pattern, toolName)) ?? false;
}
