...
export class CodexAgent extends Disposable implements IAgent {
	...
	private async _startConnection(token: string): Promise<IConnectionReady> {
		...
		const providerOverrides = [
			...
			`model_providers.vscode-proxy.name="Parakit Proxy"`,
	...
	private _registerIgnoredNotifications(client: ICodexAppServerClient): void {
		const ignored = [
			...
			'thread/settings/updated', // Parakit owns session config; Codex settings echoes are not consumed yet.
			...
			'remoteControl/status/changed', // Remote-control state is not part of the Parakit integration.
