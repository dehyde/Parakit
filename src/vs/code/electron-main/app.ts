...
import { BrowserViewKind, ipcBrowserViewChannelName, shouldOpenBrowserViewTargetInternally } from '../../platform/browserView/common/browserView.js';
...
export class CodeApplication extends Disposable {
	...
	private browserViewMainService: IBrowserViewMainService | undefined;
		...
		app.on('web-contents-created', (event, contents) => {
			...
			contents.setWindowOpenHandler(details => {
				...
				// Any other URL: delegate to OS
					...
					if (shouldOpenBrowserViewTargetInternally(details.url)) {
						const sourceWindow = this.windowsMainService?.getWindowByWebContents(contents);
						if (sourceWindow?.id !== undefined && sourceWindow.id > 0) {
							void this.browserViewMainService?.createTarget(details.url, {
								mainWindowId: sourceWindow.id,
								kind: BrowserViewKind.Browser
							}).catch(error => {
								this.logService.error('Failed to open web popup in integrated browser', error);
							});

							return { action: 'deny' };
						}

						void this.nativeHostMainService?.getActiveWindowId(-1).then(mainWindowId => {
							if (mainWindowId === undefined || mainWindowId <= 0) {
								void this.nativeHostMainService?.openExternal(undefined, details.url);
								return;
							}
	...
	private async setupProtocolUrlHandlers(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer): Promise<IInitialProtocolUrls | undefined> {
		...
		this.browserViewMainService = accessor.get(IBrowserViewMainService);
