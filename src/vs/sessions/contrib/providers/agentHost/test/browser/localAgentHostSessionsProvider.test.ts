...
suite('LocalAgentHostSessionsProvider', () => {
	...
	test('createNewSession seeds remembered values, skips unsafe keys, and ignores unscoped branches', () => {
		const storageService = disposables.add(new InMemoryStorageService());
	...
	test('createNewSession seeds branch from active session in same repository', async () => {
		const activeSession = observableValue<IActiveSession | undefined>('test.activeSession', undefined);
		const storageService = disposables.add(new InMemoryStorageService());
		const repoUri = URI.parse('file:///home/user/project');
		const provider = createProvider(disposables, agentHost, undefined, { activeSession, storageService });
		activeSession.set({
		...
		const session = provider.createNewSession(repoUri, provider.sessionTypes[0].id);
		await waitForSessionConfig(provider, session.sessionId, config => config?.values.isolation === 'worktree');

		assert.deepStrictEqual({
			remembered: storageService.getObject(STORAGE_KEY_REMEMBERED_REPO_SESSION_CONFIG_VALUES, StorageScope.PROFILE, {}),
			forwardedToAgentHost: agentHost.resolveSessionConfigRequests.at(-1)?.config?.branch,
		}, {
			remembered: {
				workspaceUri: repoUri.toString(),
				values: { [SessionConfigKey.Branch]: 'feature' },
			},
			forwardedToAgentHost: 'feature',
		});
	});
