...
import { autorun, constObservable, derived, derivedObservableWithCache, derivedOpts, IObservable, ISettableObservable, observableFromEvent, observableFromPromise, observableValue, observableValueOpts, transaction } from '../../../../../base/common/observable.js';
...
const STORAGE_KEY_REMEMBERED_REPO_SESSION_CONFIG_VALUES = 'sessions.agentHost.sessionConfigPicker.selectedRepoValues';
const UNSAFE_SESSION_CONFIG_KEYS = new Set(['__proto__', 'constructor', 'prototype']);

interface IRememberedRepoSessionConfigValues {
...
export abstract class BaseAgentHostSessionsProvider extends Disposable implements IAgentHostSessionsProvider {
	...
	constructor(
		...
		this._register(autorun(reader => {
			...
			if (!folder?.root || !branch) {
				return;
			}
			this._storeRememberedRepoSessionConfigValue(folder.root, SessionConfigKey.Branch, branch);
		}));
	}
	...
	createNewSession(workspaceUri: URI, sessionTypeId: string): ISession {
		...
		const newSession = new NewSession({
			...
			initialConfigValues: this._initialNewSessionConfig(workspaceUri),
			instantiationService: this._instantiationService,
	 ...
	 */
	protected _initialNewSessionConfig(workspaceUri: URI): Record<string, unknown> | undefined {
		...
		for (const [property, value] of Object.entries(rememberedValues)) {
			if (property !== SessionConfigKey.Branch && typeof value === 'string' && isSafeSessionConfigKey(property)) {
				config[property] = value;
			}
		}

		const rememberedBranch = this._getRememberedRepoSessionConfigValue(workspaceUri, SessionConfigKey.Branch);
		if (rememberedBranch) {
			config[SessionConfigKey.Branch] = rememberedBranch;
		}
	...
	private _getRememberedRepoSessionConfigValue(workspaceUri: URI, property: string): string | undefined {
		const remembered = this._storageService.getObject<IRememberedRepoSessionConfigValues>(STORAGE_KEY_REMEMBERED_REPO_SESSION_CONFIG_VALUES, StorageScope.PROFILE, {});
		if (typeof remembered.workspaceUri !== 'string') {
			return undefined;
		}

		let rememberedWorkspaceUri: URI;
		try {
			rememberedWorkspaceUri = URI.parse(remembered.workspaceUri);
		} catch {
			return undefined;
		}

		if (!isEqual(rememberedWorkspaceUri, workspaceUri)) {
			return undefined;
		}

		const value = remembered.values?.[property];
		return typeof value === 'string' ? value : undefined;
	}

	private _storeRememberedRepoSessionConfigValue(workspaceUri: URI, property: string, value: string): void {
		if (!isSafeSessionConfigKey(property)) {
			return;
		}

		const remembered = this._storageService.getObject<IRememberedRepoSessionConfigValues>(STORAGE_KEY_REMEMBERED_REPO_SESSION_CONFIG_VALUES, StorageScope.PROFILE, {});
		const nextValues = Object.create(null) as Record<string, string>;
		if (typeof remembered.workspaceUri === 'string') {
			try {
				if (isEqual(URI.parse(remembered.workspaceUri), workspaceUri)) {
					for (const [key, rememberedValue] of Object.entries(remembered.values ?? {})) {
						if (typeof rememberedValue === 'string' && isSafeSessionConfigKey(key)) {
							nextValues[key] = rememberedValue;
						}
					}
				}
			} catch {
				// Ignore malformed stored URIs and replace them with the current repository.
			}
		}

		nextValues[property] = value;
		this._storageService.store(STORAGE_KEY_REMEMBERED_REPO_SESSION_CONFIG_VALUES, JSON.stringify({
			workspaceUri: workspaceUri.toString(),
			values: nextValues,
		}), StorageScope.PROFILE, StorageTarget.MACHINE);
	}

	private _getRunningSessionWorkspaceUri(sessionId: string): URI | undefined {
		const rawId = this._rawIdFromChatId(sessionId);
		const cached = rawId ? this._sessionCache.get(rawId) : undefined;
		return cached?.workspace.get()?.folders[0]?.root;
	}
	...
	async setSessionConfigValue(sessionId: string, property: string, value: unknown): Promise<void> {
		...
		const newSession = this._getNewSession(sessionId);
		if (property === SessionConfigKey.Branch && typeof normalizedValue === 'string') {
			const workspaceUri = newSession?.workspaceUri ?? this._getRunningSessionWorkspaceUri(sessionId);
			if (workspaceUri) {
				this._storeRememberedRepoSessionConfigValue(workspaceUri, property, normalizedValue);
			}
		}
