...
import { GitRefType, GitRepositoryState, IGitRepository, IGitService } from '../../../../../../workbench/contrib/git/common/gitService.js';
...
function createProviderForSendTests(
	...
	opts?: { onDidCommitSession?: Event<{ original: URI; committed: URI }>; claudeEnabled?: boolean; createNewChatSessionItem?: IChatSessionsService['createNewChatSessionItem']; configurationService?: TestConfigurationService; storageService?: IStorageService; gitService?: Partial<IGitService> },
): CopilotChatSessionsProvider {
	const instantiationService = disposables.add(new TestInstantiationService());
	...
	instantiationService.stub(IGitService, opts?.gitService ?? { openRepository: async () => undefined });
	instantiationService.stub(IInstantiationService, instantiationService);
	instantiationService.stub(ILabelService, {
		getUriLabel: (uri: URI) => uri.path,
	});
...
function createGitRepository(rootUri: URI, branches: readonly string[], headName = branches[0]): IGitRepository {
	const state = observableValue<GitRepositoryState>('test.gitRepositoryState', {
		HEAD: headName ? { type: GitRefType.Head, name: headName, commit: `${headName}-commit` } : undefined,
		remotes: [],
		mergeChanges: [],
		indexChanges: [],
		workingTreeChanges: [],
	...
	return new class extends mock<IGitRepository>() {
		...
		override async getRefs() {
			return branches.map(name => ({ type: GitRefType.Head, name, commit: `${name}-commit` }));
		}
		override async diffBetweenWithStats() { return []; }
		override async diffBetweenWithStats2() { return []; }
	}();
}

async function waitForSessionBranches(session: ICopilotChatSession, expectedBranches: readonly string[]): Promise<void> {
	for (let i = 0; i < 20; i++) {
		if (session.branches.get().join('\n') === expectedBranches.join('\n')) {
			return;
		}
		await timeout(0);
	}
	assert.deepStrictEqual(session.branches.get(), expectedBranches);
}
...
suite('CopilotChatSessionsProvider', () => {
	...
	test('Copilot CLI session restores last branch for same repository', async () => {
		const storageService = disposables.add(new TestStorageService());
		const workspaceUri = URI.file('/test/project');
		const provider = createProviderForSendTests(disposables, model, async () => ({ kind: 'sent' as const, data: {} as IChatSendRequestData }), {
			storageService,
			gitService: { openRepository: async () => createGitRepository(workspaceUri, ['main', 'feature'], 'main') },
		});

		const first = provider.createNewSession(workspaceUri, CopilotCLISessionType.id);
		const firstProviderSession = provider.getSession(first.sessionId)! as ICopilotChatSession & IDisposable;
		await waitForSessionBranches(firstProviderSession, ['main', 'feature']);
		firstProviderSession.setBranch('feature');
		firstProviderSession.dispose();

		const second = provider.createNewSession(workspaceUri, CopilotCLISessionType.id);
		const secondProviderSession = provider.getSession(second.sessionId)! as ICopilotChatSession & IDisposable;
		await waitForSessionBranches(secondProviderSession, ['main', 'feature']);

		assert.strictEqual(secondProviderSession.branch.get(), 'feature');
		secondProviderSession.dispose();
	});

	test('Copilot CLI session ignores saved branch from a different repository', async () => {
		const storageService = disposables.add(new TestStorageService());
		const firstWorkspaceUri = URI.file('/test/project-a');
		const secondWorkspaceUri = URI.file('/test/project-b');
		const provider = createProviderForSendTests(disposables, model, async () => ({ kind: 'sent' as const, data: {} as IChatSendRequestData }), {
			storageService,
			gitService: {
				openRepository: async uri => createGitRepository(uri, ['main', 'feature'], 'main'),
			},
		});

		const first = provider.createNewSession(firstWorkspaceUri, CopilotCLISessionType.id);
		const firstProviderSession = provider.getSession(first.sessionId)! as ICopilotChatSession & IDisposable;
		await waitForSessionBranches(firstProviderSession, ['main', 'feature']);
		firstProviderSession.setBranch('feature');
		firstProviderSession.dispose();

		const second = provider.createNewSession(secondWorkspaceUri, CopilotCLISessionType.id);
		const secondProviderSession = provider.getSession(second.sessionId)! as ICopilotChatSession & IDisposable;
		await waitForSessionBranches(secondProviderSession, ['main', 'feature']);

		assert.strictEqual(secondProviderSession.branch.get(), 'main');
		secondProviderSession.dispose();
	});

	test('Copilot CLI session leaves branch unset when saved branch no longer exists', async () => {
		const storageService = disposables.add(new TestStorageService());
		const workspaceUri = URI.file('/test/project');
		...
		const provider = createProviderForSendTests(disposables, model, async () => ({ kind: 'sent' as const, data: {} as IChatSendRequestData }), {
			storageService,
			gitService: {
				openRepository: async () => createGitRepository(workspaceUri, branches, 'main'),
			},
		});
		...
		const second = provider.createNewSession(workspaceUri, CopilotCLISessionType.id);
		const secondProviderSession = provider.getSession(second.sessionId)! as ICopilotChatSession & IDisposable;
		await waitForSessionBranches(secondProviderSession, ['main']);

		assert.strictEqual(secondProviderSession.branch.get(), undefined);
		secondProviderSession.dispose();
	});
