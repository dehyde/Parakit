...
const STORAGE_KEY_LAST_REPO_BRANCH = 'sessions.copilotCli.lastRepoBranch';
...
class CopilotCLISession extends Disposable implements ICopilotChatSession {
	...
	private _loadBranches(repo: IGitRepository): void {
		...
		repo.getRefs({ pattern: 'refs/heads' }, cts.token).then(refs => {
			...
			if (!this._branch) {
				const storedBranch = this._getStoredBranchForRepo(repo.rootUri);
				if (storedBranch) {
					if (branches.includes(storedBranch)) {
	...
	setBranch(branch: string | undefined): void {
		if (this._branch !== branch) {
			...
			this._storeLastRepoBranch(branch);
		}
	}

	private _getStoredBranchForRepo(repoUri: URI): string | undefined {
		const raw = this.storageService.get(STORAGE_KEY_LAST_REPO_BRANCH, StorageScope.PROFILE);
		if (!raw) {
			return undefined;
		}
		try {
			const stored = JSON.parse(raw) as Partial<IStoredRepoBranch>;
			if (typeof stored.repoUri !== 'string' || typeof stored.branch !== 'string' || stored.branch.length === 0) {
				return undefined;
			}
			return isEqual(URI.parse(stored.repoUri), repoUri) ? stored.branch : undefined;
		} catch {
			return undefined;
		}
	}

	private _storeLastRepoBranch(branch: string | undefined): void {
		if (!branch || !this._repoUri) {
			return;
		}
		const stored: IStoredRepoBranch = {
			repoUri: this._repoUri.toString(),
			branch,
		};
		this.storageService.store(STORAGE_KEY_LAST_REPO_BRANCH, JSON.stringify(stored), StorageScope.PROFILE, StorageTarget.MACHINE);
	}
